...
{
	unsigned int gpio;
	int range = argc > 3 ? (name_to_gpio(argv[3]) - name_to_gpio(argv[2])) : 0;
	int args_amount = argc;
 ...
 show_usage:
	...
	if (args_amount > 3) {
		unsigned int gpio_aux = gpio;
		for (int i = 0; i <= range; i++) {
			ret = gpio_request(gpio_aux, "cmd_gpio");
			if (ret && ret != -EBUSY) {
				printf("gpio: requesting pin %u failed\n", gpio_aux);
				return -1;
			}
	...
	if (sub_cmd == GPIOC_INPUT
		...
		case GPIOC_BLK:
			if (args_amount <= 3) {
				...
				if (args_amount == 5) {
					int gpio_aux = gpio;
					int blink_range = name_to_gpio(argv[2]);
					for (int j = 0; j < blink_range; j++) {
						gpio = gpio_aux;
						for (int i = 0; i <= range; i++) {
							value = gpio_get_value(gpio);
							gpio_direction_output(gpio, !value);
		...
		if (args_amount > 3 && (sub_cmd != GPIOC_TOGGLE) && (sub_cmd != GPIOC_BLK)) {
			for (int i = 0; i <= range; i++) {
				gpio_direction_output(gpio, value);
				if (i < range) gpio++;
			}
